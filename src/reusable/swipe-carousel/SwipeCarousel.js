  ...
  return (
    <div className={className} style={containerStyle}>
      ...
      {props.map(({ x, display, sc }, i) => (
        <animated.div  {...bind()} key={i} style={{ ...outerDivStyle, display, transform: x.to(x => `translate3d(${x}px,0,0)`) }}>
          <animated.div style={{ ...titleStyle, transform: sc.to(s => `scale(${s})`) }}
            onPointerDown={() => {
              window.open(pics[index.current].link, "_blank") ||
                window.location.replace(pics[index.current].link)
            }}>
            VISIT PAGE
             </animated.div>
          <animated.div style={{ ...innerDivStyle, transform: sc.to(s => `scale(${s})`), backgroundImage: `url(${pics[i].pic})` }} />
        </animated.div>
      ))}
    </div>
  )
