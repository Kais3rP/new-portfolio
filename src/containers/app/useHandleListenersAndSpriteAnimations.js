...
export default function useHandleListenersAndSpritesAnimation(
  ...
  useEffect(() => {
    ...
    _cloudsSprite.scale.set(
      window.innerWidth / 1000,
      window.innerHeight / 1000
    );
    _cloudsSprite.position.set(0);
    _cloudsSprite.texture.baseTexture.wrapMode = PIXI.WRAP_MODES.REPEAT;
    const cloudsFilter = new PIXI.filters.DisplacementFilter(
      _cloudsSprite,
      window.innerWidth < 800 ? 50 : 50
    );
    ...
    for (let fish of allFishes) {
      ...
      fish.tlLeft = new TimelineMax()
        .to(fish.fish, { pixi: { scaleX: 0 }, duration: 0.3 })
        .to(fish.fish, { pixi: { scaleX: -0.3 }, duration: 0.1 });
      fish.tlRight = new TimelineMax()
        .to(fish.fish, { pixi: { scaleX: 0 }, duration: 0.3 })
        .to(fish.fish, { pixi: { scaleX: 0.3 }, duration: 0.1 });
    }

    //Fishes animations to follow the mouse pointer and to run back away to the starting positions
    let fishFollowTl = new TimelineMax({}).to(
      allFishes.map((fish) => fish.fish),
      10,
      {
        x: () => mousePos.x,
        y: () => mousePos.y,
        ease:
          "M0,0 C0.476,0.134 0,-0.014 0.774,0.294 0.865,0.33 0.738,0.78 1,0.986 ",
      }
    );
    ...
    function handleMouseMove(e) {
      ...
      for (let fish of allFishes) {
        if (fish.fish.position.x >= mousePos.x) {
          if (fish.direction === "right") fish.tlLeft.restart();
          fish.direction = "left";
        }
        if (fish.fish.position.x <= mousePos.x) {
          if (fish.direction === "left") fish.tlRight.restart();
  ...
  useEffect(() => {
    ...
    function handleWaterClick(e) {
      if (!isReady) return;
      const newRippleSprite = new PIXI.Sprite(
        PIXI.Loader.shared.resources.ripple.texture
      );
      ...
      const newRippleFilter = new PIXI.filters.DisplacementFilter(
        newRippleSprite
      );
    ...
    return () =>
      _firstContainer.removeListener("pointerdown", handleWaterClick);
  ...
  function handleMenuLinks(target, prevTarget) {
    ...
    if (!linkRefs[target + "LinkRef"] || !linkRefs[prevTarget + "LinkRef"])
      return;
    ...
    const diffPositionLinkContainer =
      target.getBoundingClientRect().y -
      target.parentNode.getBoundingClientRect().y;

    //Timelines
    const linkRaise = new TimelineMax().to(prevTarget, 1, {
      color: "#66ccff",
      y: 0,
      duration: 1,
      ease: "bounce",
    ...
    const linkFall = new TimelineMax().to(target, 1, {
      color: "#ff6600",
      y: containerHeight - diffPositionLinkContainer - 30,
      ease: "bounce",
    });

    if (window.innerWidth > 768)
      //Animates "Im here only" for big screens
      setIsBallDeflating((isBallDeflating) => {
        if (!isBallDeflating) inflate();
  ...
  function deflate() {
    new TimelineMax({
      onStart: () => {
        setIsBallDeflating(true);
      },
      onComplete: () => {
        setIsBallDeflating(false);
      },
    }).to(linkRefs.hereRef.current, {
      scale: 1,
      bottom: "43px",
      duration: 3.5,
      ease: "elastic",
  ...
  function inflate() {
    new TimelineMax().to(linkRefs.hereRef.current, {
      scale: "+=0.3",
      bottom: "+=10px",
      duration: 0.7,
      visibility: "visible",
      modifiers: { scaleX: checkScaleX },
    });
